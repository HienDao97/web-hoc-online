...
class StudentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(!empty($request->classroom_id)){
            Session::put('classroom_id', $request->classroom_id);
        }else{
            Session::forget('classroom_id');
        }
            ...
            ->filter(function ($query) use ($request) {
                foreach ($request->all() as $key => $value) {
                    if (($value == "") || ($value == -1) || ($value == null)) {
                        ...
                        if ($key == 'name') {
                            $query->whereRaw('LOWER(students.name) LIKE ? ', ['%'.trim(mb_strtolower($value)).'%']);
                        ...
                        }   elseif(!empty(Session::get("classroom_id"))){
                            $query->where('student_classrooms.class_room_id', Session::get("classroom_id"));
                        }
                    }
                }
            })
    ...
    public function store(Request $request){
        ...
        try {
                ...
                foreach ($params['course'] as $value){
                    if(!empty($params['classroom'][$value])){
                        foreach ($params['classroom'][$value] as $vl){
                            ...
                            $item["begin_date"] = Carbon::now();
                            $item["end_date"] = Carbon::now()->addYear();
            ...
            DB::rollback();
            Log::error('[StudentExport] ' . $e->getMessage());
    ...
    public function update(Request $request, $id)
    {
        ...
        $obj = Student::with("student_class")->where("id", $id)->whereNull('deleted_at')->first();
        if ($obj) {
            if($request->hasFile('avatar')){
                $img = $request->file('avatar')->getClientOriginalName();
                $request->avatar->move('img/user',$img);
            ...
            if(!empty($params['course'])){
                ...
                foreach ($params['course'] as $value){
                    if(!empty($params['classroom'][$value])){
                        foreach ($params['classroom'][$value] as $vl){
                            $studentClass = $obj->student_class->where("student_id",$id)->where("class_room_id", $vl)->where("course_id", $value)->first();
                            $item = [];
                            $item["class_room_id"] = $vl;
                            $item["course_id"] = $value;
                            $item["student_id"] = $id;
                            $item["status"] = (!empty($studentClass)) ? $studentClass->status :  0;
                            $item["begin_date"] = (!empty($studentClass)) ? $studentClass->begin_date : Carbon::now();
                            $item["end_date"] = (!empty($studentClass)) ? $studentClass->end_date : Carbon::now()->addYear();
                            $item["created_at"] = (!empty($studentClass)) ? $studentClass->created_at : Carbon::now();
                            array_push($array, $item);
                        }
                    }
                }
